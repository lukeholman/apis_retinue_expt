...
The information used to remove the blind coding is in the file `data/blind_codes.csv`. The blinded data collection sheet is in the file `data/blinded_behaviour_data.csv`.

Finally, the file created by this script is called `data/unblinded_behaviour_data.csv` -- this is the file to use for running new analyses, checking our conclusions, or including our data in a meta-analysis.
...
unblinded_data <- read.csv("data/blinded_behaviour_data.csv", stringsAsFactors = FALSE) %>%
  as_tibble() %>%
  filter(!is.na(tray)) %>%
  ...
  select(hive, treatment, tray, dish, number_bees, start_touch, end_touch, touch_duration) %>%
